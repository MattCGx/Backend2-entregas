...
cartRouter.post("/", passport.authenticate("jwt",{session: false}), roleValidation(['user']), cartController.createCart);

cartRouter.post("/:cid/product/:pid", passport.authenticate("jwt",{session: false}), roleValidation(['user']), cartController.addProductToCart);

cartRouter.put("/:cid", passport.authenticate("jwt",{session: false}), roleValidation(['user']), cartController.updateCart);

cartRouter.put("/:cid/product/:pid", passport.authenticate("jwt",{session: false}), roleValidation(['user']), cartController.updateProdQuantity);

cartRouter.delete("/:cid/erase", passport.authenticate("jwt",{session: false}), roleValidation(['admin']), cartController.deleteCart);

cartRouter.delete("/:cid/product/:pid", passport.authenticate("jwt",{session: false}), roleValidation(['user']), cartController.removefromCart);
